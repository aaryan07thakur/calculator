...
# It comes bundled with most Python installations, making it readily available for
#  creating desktop applications with graphical interfaces. Tkinter is based on the 
#  Tk GUI toolkit and provides a set of tools and widgets to build interactive and visually 
...
root.title("Calculator")
root.resizable(10,10)     #it is a method provided by the Tk class to control whether the window can be resized by the user.
...
#creating buttons of number
#here lambda is a way to create small, simple functions on the fly, without giving them a name. 
# Instead of using the def keyword to define a function with a name, you use lambda for quick, 
# short tasks
...
btn_equals.config(font=('verdana',12))

btn_div=Button(root,text='/',bg='#00a65a',fg='white',width=5,height=2,command=lambda:get_operator('/'))
btn_div.grid(row=4,column=3)
btn_div.config(font=('verdana'))

# In Python, especially when using the Tkinter library for creating graphical user interfaces (GUIs), 
# root.mainloop() is a line of code that is used to start the event loop of the GUI application.
